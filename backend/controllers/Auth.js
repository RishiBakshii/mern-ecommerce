...
exports.resetPassword = async (req, res) => {
    try {
        ...
        // if user does not exist then return a 404 response
        if (!isExistingUser) {
        ...
        // If token does not exist for that userId, then return a 404 response
        if (!isResetTokenExisting) {
            return res.status(404).json({ message: "Reset Link is Not Valid" });
        }

        // if the token has expired then delete the token, and send response accordingly
